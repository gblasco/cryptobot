...
# Se ejecuta desde el fichero runIndicators.py
...
class Indicators:
    ...
    def convert_to_minutes(self, interval):
        ...
        if interval in interval_mapping:
            ...
            raise ValueError("ERROR : [ Intervalo no valido. ] Por favor, usa uno de los siguientes: '1m', '5m', '30m', '1h', '1d'.")


    def dropColumns(self, df):
        df.drop(['Open', 'High', 'Low', 'Volume', 'date', 'EMA_20', 'EMA_50', 'bbm', 'bbl', 'bbh','Close', 'support', 'resistance', 'Price_change_pct'], axis=1, inplace=True)
        print(f"LOG: [Indicators Calculated]:  Drop not necessary columns...")   
        return df
    

    def getEMA(self, df):
        # hacer calculo de SMA tambien. En principio descartado hacer SMA pq el modelo funciona bien
        ...
        # Pos relativa del precio respecto a las EMAs
        ...
        # Proporcion del precio respecto a las EMAs
    ...
    def addNums(self, df):
        # funcion para convertir para columnas booleanas a (1 o 0)
        bool_columns = [
    ...
    def addRSI(self, df):
        ...
        df['RSI_change'] = df['RSI'].diff()  # diferencia con respecto al anterior RSI
        return df
    ...
    def addBollinger(self, df):
        ...
        df['bbm'] = bb.bollinger_mavg()   # Banda media
        df['bbh'] = bb.bollinger_hband()  # Banda superior
        df['bbl'] = bb.bollinger_lband()  # Banda inferior
        # Cruces
        ...
        # ratios
        df['price_to_bbh_ratio'] = df['Close'] / df['bbh']  # ratio respecto a la banda superior
        df['price_to_bbl_ratio'] = df['Close'] / df['bbl']  # ratio respecto a la banda inferior
        df['price_to_bbm_ratio'] = df['Close'] / df['bbm']  # ratio respecto a la banda media
        return df
    ...
    def addCloseDeltas(self, df):
        df['Close_5min_pct_change'] = df['Close'].pct_change(1) * 100  # Cambio porcentual desde hace 5 minutos (1 periodo)
        df['Close_30min_pct_change'] = df['Close'].pct_change(6) * 100  # Cambio porcentual desde hace 30min (6 periodos)
        df['Close_1h_pct_change'] = df['Close'].pct_change(12) * 100  # 12 periodos de 5 min cada uno
        df['Close_6h_pct_change'] = df['Close'].pct_change(72) * 100  # 72 periodos de 5 min cada uno
        df['Close_24h_pct_change'] = df['Close'].pct_change(288) * 100  # 288 periodos de 5 min cada uno
        return df
    ...
    def getIndicatorsCalculated(self, interval, windowrs, look_ahead_intervals, pct):
        df = pd.read_csv(f'../data/history/fullhistory{interval}.csv')
        df = self.getEMA(df)
        #df = self.getSMA(df)
        df = self.addRSI(df)
        df = self.addMACD(df)
        ...
        df = self.addTarget(df, look_ahead_intervals, pct)
        ...
        df.to_csv(os.path.join('../data/history', 'fullhistorywithindicators' + interval + '_up02pct.csv'), index=False)
        print(f"LOG: [Indicators Calculated]:  ../data/history/fullhistorywithindicators{interval}_up02pct.csv")
        return df
